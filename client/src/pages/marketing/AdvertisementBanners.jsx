...
import {
  ...
  CheckCircle, AlertCircle, Settings, Megaphone, Zap, Target, ArrowUpRight, ArrowDownRight, Star, Home, Sprout, Banknote, Sparkles, CloudRain, Store, Leaf, Bot
} from 'lucide-react'
...
const BANNERS = [
  { id: 'BNR-001', name: 'Kharif Season Fertilizer Offer', type: 'Homepage Hero', start: 'Jun 1, 2026', end: 'Aug 31, 2026', impressions: '540,000', clicks: '12,800', ctr: '2.4%', status: 'Scheduled', img: Sprout },
  { id: 'BNR-002', name: 'Monsoon Pesticide Discount 20%', type: 'Promotional Banner', start: 'May 25, 2026', end: 'Jun 15, 2026', impressions: '820,000', clicks: '18,200', ctr: '2.2%', status: 'Active', img: CloudRain },
  { id: 'BNR-003', name: 'New Vendor Welcome Banner', type: 'Offer Banner', start: 'Ongoing', end: 'Ongoing', impressions: '340,000', clicks: '7,400', ctr: '2.2%', status: 'Active', img: Store },
  { id: 'BNR-004', name: 'Rabi Season Crop Bundle', type: 'Seasonal Campaign', start: 'Oct 1, 2026', end: 'Dec 31, 2026', impressions: '0', clicks: '0', ctr: '—', status: 'Paused', img: Leaf },
  { id: 'BNR-005', name: 'AI Disease Detection Feature', type: 'Feature Banner', start: 'May 1, 2026', end: 'Jun 30, 2026', impressions: '210,000', clicks: '5,400', ctr: '2.6%', status: 'Active', img: Bot },
]
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className={card}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            { type: 'Homepage Banners', count: 3, icon: Home, color: 'from-pink-500/10 to-rose-500/10 border-pink-500/20' },
            { type: 'Promotional Banners', count: 5, icon: Target, color: 'from-orange-500/10 to-amber-500/10 border-orange-500/20' },
            { type: 'Seasonal Campaigns', count: 4, icon: Sprout, color: 'from-emerald-500/10 to-green-500/10 border-emerald-500/20' },
            { type: 'Offer Banners', count: 6, icon: Banknote, color: 'from-blue-500/10 to-indigo-500/10 border-blue-500/20' },
            { type: 'Feature Banners', count: 2, icon: Sparkles, color: 'from-purple-500/10 to-violet-500/10 border-purple-500/20' },
          ].map((cat, i) => (
            <motion.div key={i} whileHover={{ scale: 1.03 }} className={`p-3 rounded-xl border bg-gradient-to-br ${cat.color} cursor-pointer`}>
              <div className="text-lg mb-1">{React.createElement(cat.icon, { size: 20, className: "text-slate-600 dark:text-slate-300" })}</div>
                ...
                <motion.tr key={b.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-pink-500/20 to-purple-500/20 border border-pink-500/20 flex items-center justify-center text-base">{React.createElement(b.img, { size: 16, className: "text-pink-500" })}</div>
                      <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`${card} lg:col-span-2`}>
          ...
          <div className="space-y-3">
            {[
              { name: 'Monsoon Pesticide Discount 20%', impressions: '820K', clicks: '18.2K', ctr: '2.22%', bg: CloudRain },
              { name: 'AI Disease Detection Feature', impressions: '210K', clicks: '5.4K', ctr: '2.57%', bg: Bot },
              { name: 'New Vendor Welcome Banner', impressions: '340K', clicks: '7.4K', ctr: '2.18%', bg: Store },
            ].map((p, i) => (
              <div key={i} className={`p-3.5 rounded-xl border flex items-center gap-4 ${isDark ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-pink-500/20 to-purple-500/20 flex items-center justify-center text-xl">{React.createElement(p.bg, { size: 20, className: "text-pink-500" })}</div>
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="w-full h-40 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 flex items-center justify-center mb-4">
                <div className="text-center text-white">
                  <div className="flex justify-center mb-2">{React.createElement(previewBanner.img, { size: 48, className: "text-white" })}</div>
