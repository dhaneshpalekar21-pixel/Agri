                  ...
                  >
                    {group.items.map((subItem) => {
                      ...
                      const SUBSCRIPTION_ROUTES = {
                        'Premium Plans': '/subscription/premium-plans',
                        'Vendor Memberships': '/subscription/vendor-memberships',
                        'Farmer Plans': '/subscription/farmer-plans',
                        'Billing Cycles': '/subscription/billing-cycles',
                        'Plan Features': '/subscription/plan-features',
                        'Subscription Analytics': '/subscription/analytics'
                      }
                      const SCHEME_ROUTES = {
                        'Government Schemes': '/schemes/directory',
                        'Farmer Eligibility': '/schemes/eligibility',
                        'Subsidy Tracking': '/schemes/subsidy-tracking',
                        'Application Support': '/schemes/support',
                        'Scheme Notifications': '/schemes/notifications'
                      }
                      const INTEGRATION_ROUTES = {
                        'Payment Gateway APIs': '/integrations/payment-gateways',
                        'SMS APIs': '/integrations/sms',
                        'WhatsApp APIs': '/integrations/whatsapp',
                        'Google Maps API': '/integrations/google-maps',
                        'Weather APIs': '/integrations/weather',
                        'AI APIs': '/integrations/ai',
                        'ERP Integrations': '/integrations/erp',
                        'IoT Integrations': '/integrations/iot'
                      }
                      const SYSTEM_ROUTES = {
                        'Logs': '/system-tools/logs',
                        'Cache Management': '/system-tools/cache',
                        'Database Backup': '/system-tools/backup',
                        'Server Monitoring': '/system-tools/server',
                        'Error Reports': '/system-tools/errors',
                        'API Monitoring': '/system-tools/api'
                      }
                      return (
                        <button
                          key={subItem}
                          onClick={() => {
                            ...
                            } else if (group.category === 'Subscription & Plans' && SUBSCRIPTION_ROUTES[subItem]) {
                              navigate(SUBSCRIPTION_ROUTES[subItem])
                            } else if (group.category === 'Government Schemes' && SCHEME_ROUTES[subItem]) {
                              navigate(SCHEME_ROUTES[subItem])
                            } else if (group.category === 'Integrations' && INTEGRATION_ROUTES[subItem]) {
                              navigate(INTEGRATION_ROUTES[subItem])
                            } else if (group.category === 'System Tools' && SYSTEM_ROUTES[subItem]) {
                              navigate(SYSTEM_ROUTES[subItem])
                            } else {
                              navigate('/dashboard')
                            }
